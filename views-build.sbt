...
lazy val `views-test` = taskKey[Unit]("Run Views tests when testing application.")

`views-test` := {
  implicit val userInterfaceRoot = baseDirectory.value / "views"
  if (executeUiTests != Success) throw new Exception("Views tests failed!")
}

lazy val `views-prod-build` = taskKey[Unit]("Run Views build when packaging the application.")

`views-prod-build` := {
  implicit val userInterfaceRoot = baseDirectory.value / "views"
  if (executeProdBuild != Success) throw new Exception("Oops! Views Build crashed.")
}

// Execute frontend prod build task prior to play dist execution.
dist := (dist dependsOn `views-prod-build`).value

// Execute frontend prod build task prior to play stage execution.
stage := (stage dependsOn `views-prod-build`).value

// Execute frontend test task prior to play test execution.
test := ((test in Test) dependsOn `views-test`).value
